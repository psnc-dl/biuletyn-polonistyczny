...
class Command(BaseCommand):
    args = ''
    help = 'IMPORT DISSERTATIONS FROM CSV FILE'
    ...
    def dissertationsImport(self, fname):
        start = datetime.datetime.now()
        
        imported_data = Dataset().load(open(fname).read())
        dissertation_resource = DissertationResource()
        result = dissertation_resource.import_data(imported_data, dry_run=False)
    ...
    def handle(self, *args, **options):
        logger.info('--- IMPORT DISSERTATION DATA ---')
        logger.info('-------------------------')
        logger.info('Import started...')
        logger.info(' ')
        if options['file']:
            self.dissertationsImport(options['file'])
        logger.info(' done.')
        logger.info(' ')
        logger.info('-------------------------')
